...
class WasteTypesController extends Controller
{
    ...
    {
        $data = WasteTypes::whereSpaceId(\Auth::id())->where(static function ($query) use ($request) {
            if ($request->get('name')) {
        ...
        $id = $request->get('edit');
        if ($id === null) {
            $model = new WasteTypes();
        } else {
            $model = WasteTypes::where('id', $id)->first();
            if ($model && $model->space_id !== \Auth::user()->space_id) {
                return redirect()->route('wasteTypes.index');
            }
        }
        if (!$model) {
            return redirect()->route('wasteTypes.index');
        ...
        return view('panel.wasteTypes.index', [
            'data'       => $data->paginate(15),
            'item'       => $model,
            'path'       => $path,
            'searchName' => $request->get('name'),
            'breadcrumb' => [
                'title' => __('sidebar.wasteTypes'),
                'bc'    =>
                    [
                        [
                            'link'   => route('main'),
                            ...
                            'link'   => route('wasteTypes.index'),
                            'name'   => __('sidebar.wasteTypes'),
                            'active' => true
                        ],
                    ]
            ]
    ...
    public function save(Request $request, ?int $id = null): RedirectResponse
    {
        $validator = Validator::make($request->post(), [
            ...
            'name.required' => __('wasteTypes.errors.name'),
        ]);
        ...
        return redirect()->route('wasteTypes.index')->with('success', __('wasteTypes.save'));
    }
    ...
    public function delete($id): RedirectResponse
    {
        ...
        $model->delete();
        return redirect()->route('wasteTypes.index')->with('success', __('wasteTypes.deleted'));
    }
}
