...
class ClientTypeController extends Controller
{
    ...
    {
        $data = ClientType::whereSpaceId(\Auth::id());
        if ($request->get('name')) {
        ...
        if ($id === null) {
            ...
            $model = ClientType::where('id', $id)->first();
        ...
        return view('panel.clientType.index', [
            'data'       => $data->paginate(15),
            'item'       => $model,
            'path'       => $path,
            'searchName' => $request->get('name'),
            'breadcrumb' => [
                'title' => __('sidebar.clientType'),
                'bc'    =>
                    [
                        [
                            'link'   => route('main'),
                            ...
                            'link'   => route('clientType.index'),
                            'name'   => __('sidebar.clientType'),
                            'active' => true
                        ],
                    ]
            ]
    ...
    public function save(Request $request, ?int $id = null): RedirectResponse
    {
        $validator = Validator::make($request->post(), [
            ...
            'title.required' => __('clientType.errors.name'),
        ]);
        ...
        return redirect()->route('clientType.index')->with('success', __('clientType.save'));
    }
    ...
    public function delete($id): RedirectResponse
    {
        ...
        $model->delete();
        return redirect()->route('clientType.index')->with('success', __('clientType.deleted'));
    }
}
