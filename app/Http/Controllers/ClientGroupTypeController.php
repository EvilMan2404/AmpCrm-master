...
class ClientGroupTypeController extends Controller
{
    /**
    ...
    {
        $data = ClientGroupType::whereSpaceId(\Auth::id());
        if ($request->get('name')) {
        ...
        $id = $request->get('edit');
        if ($id === null) {
            $model = new ClientGroupType();
        } else {
            $model = ClientGroupType::where('id', $id)->first();
            if ($model && $model->space_id !== Auth::user()->space_id) {
                return redirect()->route('clientGroupType.index');
            }
        }
        if (!$model) {
            return redirect()->route('clientGroupType.index');
        ...
        return view('panel.clientGroupType.index', [
            'data'       => $data->paginate(15),
            'item'       => $model,
            'path'       => $path,
            'searchName' => $request->get('name'),
            'breadcrumb' => [
                'title' => __('sidebar.clientGroupType'),
                'bc'    =>
                    [
                        [
                            'link'   => route('main'),
                            ...
                            'link'   => route('clientGroupType.index'),
                            'name'   => __('sidebar.clientGroupType'),
                            'active' => true
                        ],
                    ]
            ]
    ...
    public function save(Request $request, ?int $id = null): RedirectResponse
    {
        $validator = Validator::make($request->post(), [
            ...
            'title.required' => __('clientGroupType.errors.name'),
        ]);
        ...
        return redirect()->route('clientGroupType.index')->with('success', __('clientGroupType.save'));
    }
    ...
    public function delete($id): RedirectResponse
    {
        ...
        $model->delete();
        return redirect()->route('clientGroupType.index')->with('success', __('clientGroupType.deleted'));
    }
}
